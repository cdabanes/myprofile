...
class EmployeesController extends AppController {
	...
	var $name = 'Employees';
	var $uses =  array('Employee','SystemCounter','Accounting.Organization');
	...
	function index() {
		if ($this->Rest->isActive()) {
			...
			if($this->RequestHandler->isAjax()){	
				...
				$this->set('organizations', $this->Organization->find('list',array('fields'=>array('division_code','description'))));
	...
	function add() {
		if (!empty($this->data)) {
			...
			if(empty($this->data['Employee']['id'])){
			...
			if ($this->Employee->save($this->data)) {
				...
				if($this->RequestHandler->isAjax()){
					$response['status'] = -1;
					$response['msg'] = '<i class="icon-search"></i>&nbsp; The Employee could not be saved. Please, try again.';
					$response['data'] = $this->data;
					echo json_encode($response);
					exit();
				}else{ 
		...
		$Employee = $this->Employee->Employee->find('list');
	}
