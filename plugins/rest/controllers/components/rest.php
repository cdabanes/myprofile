	...
	public function credentials ($set = false) {
		...
		if ($set === true) {
			if (!empty($_SERVER['HTTP_AUTHORIZATION'])) {
				...
				foreach($parts as $p){
					$v = explode('=',$p);
					$creds[$v[0]] = str_replace(',','',str_replace('"','',$v[1]));
				}
				$joins = array(
						array(
						'table' => 'oauth_server_registry',
						'alias' => 'ServerRegistry',
						'type' => 'inner',
						'conditions' => array('ServerRegistry.osr_usa_id_ref = User.id')
						)
				);
				$user = $this->User()->find('first',array('joins'=>$joins,'conditions'=>array('ServerRegistry.osr_consumer_key'=>$creds['oauth_consumer_key'])));
				$parts['username']= $user['User']['username'];
				$parts['apikey']= $creds['oauth_token'];
				$parts['class']= 'Employee';
	...
	public function response ($data = array()) {
		...
		$response = array(
			'meta' => array(
				...
				'status' => $status,
				'feedback' => $feedback,
				'token'=>isset($creds['apikey'])?$creds['apikey']:'No token'
			)
		);
